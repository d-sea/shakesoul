...
function cforms_dashboard() {
	...
	if( count($entries)>0 ){
		foreach($entries as $entry)
				$content.= "<li>".$cformsSettings['form'.$no]['cforms'.$entry->form_id.'_fname']." [<a href='admin.php?page=".$plugindir."/cforms-database.php&d-id=$entry->id#entry$entry->id'>$entry->email</a>] @ $entry->sub_date</li>";
	}
	else
