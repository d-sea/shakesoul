...
function get_captcha_uri() {
	...
	$c1 = prep( $cap['c1'],'3' );
	$c2 = prep( $cap['c2'],'5' );
	$ac = prep( $cap['ac'],'abcdefghijkmnpqrstuvwxyz23456789' );
	$i = prep( $cap['i'],'' );
	$h = prep( $cap['h'],25 );
	$w = prep( $cap['w'],115 );
	$c = prep( $cap['c'],'000066' );
...
function rc() {
	global $cformsSettings;
	$c = $cformsSettings['cforms_captcha_def'];
	$min = prep( $c['c1'],4 );
	$max = prep( $c['c2'],5 );
	$src = prep( $c['ac'], 'abcdefghijkmnpqrstuvwxyz23456789');
	...
	function outputRSS() {
		...
		if( $temp <> '' ) {
			...
			if( $all || $single ){
				...
				$WHERE='';
				if( $all )
					$rsscount = $cformsSettings['global']['cforms_rssall_count'];
				else if( $single ){
					$WHERE = "WHERE form_id = '".$no."'";
					$rsscount = $cformsSettings['form'.$no]['cforms'.$no.'_rss_count'];
				}
