...
function reset_captcha( $no = '' ){
	...
	$captcha = $cformsSettings['global']['cforms_captcha_def'];
	if ( $captcha['i']=='i' )
		$tu = strtolower($_SESSION['turing_string_'.$no]);
	else
		$tu = $_SESSION['turing_string_'.$no];

	$newimage = md5($tu).'|'.$no.'|'.$path.'/cforms-captcha.php?ts='.$no.str_replace('&amp;','&',get_captcha_uri());
	return $newimage;
}
...
function cforms_submitcomment($content) {
	...
	for($i = 1; $i <= sizeof($params)-2; $i++) {
			...
			while ( in_array($field_stat[1],array('cauthor','email','url')) && $user->ID ) {
					...
					$htmlformdata .= '<tr><td style=3D"'.$cformsSettings['global']['cforms_style_data_td'].'">' . $field_stat[1] . '</td><td>' . $track[$field_stat[1]] . '</td></tr>' . "\n";
