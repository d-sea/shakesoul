...
<div class="wrap" id="top">
			...
			<div class="chgformbox">
				...
				<div class="chgR">
			      ...
			      if ( (int)$cformsSettings['global']['cforms_formcount'] > 1)
			        echo '<input class="allbuttons deleteall" title="'.__('This will delete the current form - no warning!', 'cforms').'" type="submit" onclick="return confirm(\''.__('This will delete the current form!', 'cforms').'\')" name="delbutton" value="'.__('Delete THIS Form(!)', 'cforms').'"/>';
			      ?>
				</div>
				<div class="chgM">
					<?php
	...
	<fieldset id="anchorfields">
		...
		<p class="mainoptions">
			...
			<input id="cforms_ajax" type="checkbox" class="cforms_ajax" name="cforms_ajax" <?php if($cformsSettings['form'.$no]['cforms'.$no.'_ajax']=="1") echo "checked=\"checked\""; ?>/>
						...
						<div id="f<?php echo $i; ?>" class="groupItem<?php echo $alternate; ?>">
							...
							<div class="itemContent">
								...
								<input title="<?php _e('Please enter field definition', 'cforms'); ?>" class="inpfld" <?php echo $specialclass; ?> name="field_<?php echo($i); ?>_name" id="field_<?php echo($i); ?>_name" size="30" value="<?php echo ($field_type == 'fieldsetend')?'--':$field_name; ?>" /><a href="#" class="jqModal" title="<?php echo $cforms_root.'/js/include/'; ?>"><img class="wrench" src="<?php echo $cforms_root; ?>/images/wrench.gif" alt="<?php _e('edit', 'cforms'); ?>" title="<?php _e('edit', 'cforms'); ?>"/></a><select title="<?php _e('Pick a field type', 'cforms'); ?>" class="fieldtype selfld" <?php echo $specialclass; ?> name="field_<?php echo($i); ?>_type" id="field_<?php echo($i); ?>_type">
									...
									<option<?php echo $dis; ?> value="" class="disabled" disabled="disabled"><?php _e('----- T-A-F form fields ------', 'cforms'); ?></option>
									...
									<option<?php echo $dis; ?> value="" class="disabled" disabled="disabled"><?php _e('--- WP comment form fields ---', 'cforms'); ?></option>
									...
									<?php endif; ?>
									<?php if ( function_exists('comment_luv') ) : ?>
								...
								if( in_array($field_type,array('hidden','checkboxgroup', 'radiobuttons','fieldsetstart','fieldsetend','ccbox','captcha','verification','textonly')) )
									...
									echo '<input class="fieldisreq chkfld" type="checkbox" title="'.__('input required', 'cforms').'" name="field_'.($i).'_required" value="required"'.($field_required == '1'?' checked="checked"':'').'/>';


								if( ! in_array($field_type,array('textfield','youremail','friendsemail','email')) )
									echo '<img class="chkno" src="'.$cforms_root.'/images/chkbox_grey.gif" alt="'.__('n/a', 'cforms').'" title="'.__('Not available.', 'cforms').'"/>';
								else
									echo '<input class="fieldisemail chkfld" type="checkbox" title="'.__('email required', 'cforms').'" name="field_'.($i).'_emailcheck" value="required"'.($field_emailcheck == '1'?' checked="checked"':'').'/>';
								...
								if( ! in_array($field_type,array('pwfield','textarea','textfield','datepicker','yourname','youremail','friendsname','friendsemail','email','author','url','comment')) )
									...
									echo '<input class="fieldclear chkfld" type="checkbox" title="'.__('clear field', 'cforms').'" name="field_'.($i).'_clear" value="required"'.($field_clear == '1'?' checked="checked"':'').'/>';
								...
								if( ! in_array($field_type,array('pwfield','textarea','textfield','datepicker','checkbox','checkboxgroup','selectbox','multiselectbox','radiobuttons','upload')) )
									...
									echo '<input class="fielddisabled chkfld" type="checkbox" title="'.__('disabled', 'cforms').'" name="field_'.($i).'_disabled" value="required"'.($field_disabled == '1'?' checked="checked"':'').'/>';


								if( ! in_array($field_type,array('pwfield','textarea','textfield','datepicker','checkbox','checkboxgroup','selectbox','multiselectbox','radiobuttons','upload')) )
									echo '<img class="chkno" src="'.$cforms_root.'/images/chkbox_grey.gif" alt="'.__('n/a', 'cforms').'" title="'.__('Not available.', 'cforms').'"/>';
								else
									echo '<input class="fieldreadonly chkfld" type="checkbox" title="'.__('read-only', 'cforms').'" name="field_'.($i).'_readonly" value="required"'.($field_readonly == '1'?' checked="checked"':'').'/>';
	...
	<fieldset id="fileupload" class="cformsoptions <?php if( !$fileupload) echo "hidden"; ?>">
			...
			<div id="o0">
				...
				<div class="optionsbox" style="margin-top:15px;">
					...
					<div class="optionsboxR"><input type="text" id="cforms_upload_ext" name="cforms_upload_ext" value="<?php echo stripslashes(htmlspecialchars($cformsSettings['form'.$no]['cforms'.$no.'_upload_ext'])); ?>"/> <?php _e('[empty=all files are allowed]', 'cforms') ?></div>
				</div>
				...
				<div class="optionsbox" style="margin-top:3px;">
					...
					<div class="optionsboxR"><input type="text" id="cforms_upload_size" name="cforms_upload_size" value="<?php echo stripslashes(htmlspecialchars($cformsSettings['form'.$no]['cforms'.$no.'_upload_size'])); ?>"/></div>
				</div>

				<div class="optionsbox">
					<div class="optionsboxL"><label for="cforms_noattachments"><strong><?php _e('Do not email attachments', 'cforms') ?></strong></label></div>
					<div class="optionsboxR"><input type="checkbox" id="cforms_noattachments" name="cforms_noattachments" <?php if($cformsSettings['form'.$no]['cforms'.$no.'_noattachments']=="1") echo "checked=\"checked\""; ?>/><br /><?php echo sprintf(__('<u>Note</u>: Attachments are stored on the server &amp; can be accessed via the <a href="%s" %s>cforms tracking</a> tables.', 'cforms'),'?page='. $plugindir.'/cforms-global-settings.php#tracking','onclick="setshow(14)"'); ?></div>
				</div>
		...
		<fieldset class="cformsoptions" id="anchormessage">
			...
			<div id="o1">
				...
				<div class="optionsbox">
					...
					<div class="optionsboxR"><input name="cforms_submit_text" id="cforms_submit_text" value="<?php echo ($cformsSettings['form'.$no]['cforms'.$no.'_submit_text']);  ?>" /></div>
				</div>
				...
				<div class="optionsbox" style="margin-top:10px;">
					...
					<div class="optionsboxR"><input name="cforms_working" id="cforms_working" value="<?php echo stripslashes(htmlspecialchars($cformsSettings['form'.$no]['cforms'.$no.'_working']));  ?>" /></div>
				</div>
				<div class="optionsbox" style="margin-top:10px;">
					<div class="optionsboxL"><label for="cforms_required"><strong><?php _e('"required" label', 'cforms'); ?></strong></label></div>
					<div class="optionsboxR"><input type="text" name="cforms_required" id="cforms_required" value="<?php echo stripslashes(htmlspecialchars($cformsSettings['form'.$no]['cforms'.$no.'_required'])); ?>"/></div>
				</div>
				<div class="optionsbox">
					<div class="optionsboxL"><label for="cforms_emailrequired"><strong><?php _e('"email required" label', 'cforms'); ?></strong></label></div>
					<div class="optionsboxR"><input type="text" name="cforms_emailrequired" id="cforms_emailrequired" value="<?php echo stripslashes(htmlspecialchars($cformsSettings['form'.$no]['cforms'.$no.'_emailrequired'])); ?>"/></div>
				</div>

				<div class="optionsbox" style="margin-top:10px;">
					<div class="optionsboxL"><label for="cforms_success"><?php _e('<strong>Success message</strong><br />when form filled out correctly', 'cforms'); ?></label></div>
					<div class="optionsboxR">
						<div id="r1" class="rbox" style="float:left"><textarea rows="80px" cols="200px" name="cforms_success" id="cforms_success"><?php echo stripslashes(htmlspecialchars($cformsSettings['form'.$no]['cforms'.$no.'_success'])); ?></textarea><div id="rh1"></div></div>
						<div style="float:left"><input type="checkbox" id="cforms_popup1" name="cforms_popup1" <?php if(substr($cformsSettings['form'.$no]['cforms'.$no.'_popup'],0,1)=="y") echo "checked=\"checked\""; ?>/><label for="cforms_popup1"><?php _e('Opt. Popup Msg', 'cforms'); ?></label></div>
					</div>
				</div>

				<div class="optionsbox">
					<div class="optionsboxL"><label for="cforms_failure"><?php _e('<strong>Failure message</strong><br />when missing fields or wrong field<br />formats (regular expr.)', 'cforms'); ?></label></div>
					<div class="optionsboxR">
						<div id="r2" class="rbox" style="float:left"><textarea rows="80px" cols="200px" name="cforms_failure" id="cforms_failure" ><?php echo stripslashes(htmlspecialchars($cformsSettings['form'.$no]['cforms'.$no.'_failure'])); ?></textarea><div id="rh2"></div></div>
						<div style="float:left"><input type="checkbox" id="cforms_popup2" name="cforms_popup2" <?php if(substr($cformsSettings['form'.$no]['cforms'.$no.'_popup'],1,1)=="y") echo "checked=\"checked\""; ?>/><label for="cforms_popup2"><?php _e('Opt. Popup Msg', 'cforms'); ?></label></div>
					</div>
				</div>
				<div class="optionsbox" style="margin-top:10px;">
					<div class="optionsboxL"><label for="cforms_showposa"><strong><?php _e('Show messages', 'cforms'); ?></strong></label></div>
					<div class="optionsboxR">
						<input type="checkbox" id="cforms_showposa" name="cforms_showposa" <?php if(substr($cformsSettings['form'.$no]['cforms'.$no.'_showpos'],0,1)=="y") echo "checked=\"checked\""; ?>/><label for="cforms_showposa"><?php _e('Above form', 'cforms'); ?></label><br />
						<input type="checkbox" id="cforms_showposb" name="cforms_showposb" <?php if(substr($cformsSettings['form'.$no]['cforms'.$no.'_showpos'],1,1)=="y") echo "checked=\"checked\""; ?>/><label for="cforms_showposb"><?php _e('Below form', 'cforms'); ?></label>
					</div>
				</div>

				<div class="optionsboxSEP"></div>

				<div class="optionsbox">
					<div class="optionsboxL"><label for="cforms_jump"><strong><?php _e('Jump to Error', 'cforms'); ?></strong></label></div>
					<div class="optionsboxR">
						<input type="checkbox" id="cforms_jump" name="cforms_jump" <?php if(substr($cformsSettings['form'.$no]['cforms'.$no.'_showpos'],4,1)=="y") echo "checked=\"checked\""; ?>/><label for="cforms_jump"><?php _e('(Only Javascript)', 'cforms'); ?></label>
					</div>
				</div>
				<div class="optionsbox">
					<div class="optionsboxL"><label for="cforms_errorLI"><strong><?php _e('Fancy Error messages', 'cforms'); ?></strong></label></div>
					<div class="optionsboxR">
						<input type="checkbox" id="cforms_errorLI" name="cforms_errorLI" <?php if(substr($cformsSettings['form'.$no]['cforms'.$no.'_showpos'],2,1)=="y") echo "checked=\"checked\""; ?>/><label for="cforms_errorLI"><?php _e('Enhanced display of errors (see also new theme CSS classes)', 'cforms'); ?></label>
					</div>
				</div>
				<div class="optionsbox">
					<div class="optionsboxL"><label for="cforms_errorINS"><strong><?php _e('Embedded Custom Error<br />Messages', 'cforms'); ?></strong></label></div>
					<div class="optionsboxR">
						<input type="checkbox" id="cforms_errorINS" name="cforms_errorINS" <?php if(substr($cformsSettings['form'.$no]['cforms'.$no.'_showpos'],3,1)=="y") echo "checked=\"checked\""; ?>/><label for="cforms_errorINS"><?php _e('Field specific error messages will be shown above input field', 'cforms'); ?></label>
					</div>
				</div>
		...
		<fieldset class="cformsoptions" id="anchoremail">
			...
			<div id="o2">
				...
				<div class="optionsbox">
					...
					<div class="optionsboxR"><input type="text" name="cforms_fromemail" id="cforms_fromemail" value="<?php echo stripslashes(htmlspecialchars($cformsSettings['form'.$no]['cforms'.$no.'_fromemail'])); ?>" /></div>
				</div>

				<div class="optionsbox" style="margin-top:15px;">
					<div class="optionsboxL"><label for="cforms_email"><strong><?php _e('Admin email address(es)', 'cforms') ?></strong></label></div>
					<div class="optionsboxR"><input type="text" name="cforms_email" id="cforms_email" value="<?php echo stripslashes(htmlspecialchars($cformsSettings['form'.$no]['cforms'.$no.'_email'])); ?>" /> <a class="infobutton" href="#" name="it1"><?php _e('More than one "<strong>form admin</strong>"? &raquo;', 'cforms'); ?></a></div>
				</div>
				...
				<div class="optionsbox">
					...
					<div class="optionsboxR"><input type="text" name="cforms_bcc" id="cforms_bcc" value="<?php echo stripslashes(htmlspecialchars($cformsSettings['form'.$no]['cforms'.$no.'_bcc'])); ?>" /></div>
				</div>
				<div class="optionsbox">
					<div class="optionsboxL"><label for="cforms_subject"><strong><?php _e('Subject admin email', 'cforms') ?></strong></label></div>
					<div class="optionsboxR"><input type="text" name="cforms_subject" id="cforms_subject" value="<?php echo stripslashes(htmlspecialchars($cformsSettings['form'.$no]['cforms'.$no.'_subject'])); ?>" /> <?php echo sprintf(__('<a href="%s" %s>Variables</a> allowed.', 'cforms'),'?page='. $plugindir.'/cforms-help.php#variables','onclick="setshow(23)"'); ?></div>
				</div>

				<?php if( $cformsSettings['global']['cforms_showdashboard'] == '1' ) : ?>
					<div class="optionsbox" style="margin-top:15px;">
						<div class="optionsboxL"></div>
						<div class="optionsboxR"><input type="checkbox" id="cforms_dashboard" name="cforms_dashboard" <?php if($o=$cformsSettings['form'.$no]['cforms'.$no.'_dashboard']=="1") echo "checked=\"checked\""; ?>/><label for="cforms_dashboard"><?php _e('Include in <strong>last 5 entries shown</strong> on the dashboard.', 'cforms') ?></label></div>
					</div>
				<?php endif; ?>
				...
				<div class="optionsbox">
					...
					<div class="optionsboxR"><input type="checkbox" id="cforms_customnames" name="cforms_customnames" <?php if($cformsSettings['form'.$no]['cforms'.$no.'_customnames']=='1') echo "checked=\"checked\""; ?>/> <a class="infobutton" href="#" name="it4"><?php _e('Please read note &raquo;', 'cforms'); ?></a></div>
				</div>
				...
				<div class="optionsbox" style="margin-top:10px;">
					...
					<div class="optionsboxR"><input type="checkbox" id="cforms_taftrick" name="cforms_taftrick" <?php if($cformsSettings['form'.$no]['cforms'.$no.'_tellafriend']=='3') echo "checked=\"checked\""; ?>/> <a class="infobutton" href="#" name="it5"><?php _e('Please read note &raquo;', 'cforms'); ?></a></div>
				</div>
				...
				<div class="optionsbox">
					...
					<div class="optionsboxR"><input type="text" id="cforms_maxentries" name="cforms_maxentries" value="<?php echo stripslashes(htmlspecialchars($cformsSettings['form'.$no]['cforms'.$no.'_maxentries'])); ?>"/><label for="cforms_maxentries"><?php _e('<u>total</u> # of submissions accepted [<strong>empty or 0 (zero) = off</strong>] (tracking must be enabled!)', 'cforms') ?></label></div>
				</div>

				<div class="optionsbox">
					<div class="optionsboxL" style="padding-top:7px"><strong><?php _e('Start Date', 'cforms'); ?></strong></div>
					...
					<div class="optionsboxR">
                        ...
                        if( strlen($cformsSettings['form'.$no]['cforms'.$no.'_startdate'])>1 ):
							...
							if( $dt>0 ):
	                                echo '<label for="cforms_startdate">'.__('The form will be available in ', 'cforms').sec2hms($dt).'.</label></div>';
	                            else:
	                                echo '<label for="cforms_startdate">'.__('The form is available now.', 'cforms').'</label></div>';
							endif;
						endif;
                        ?>
				...
				<div class="optionsbox">
					<div class="optionsboxL" style="padding-top:7px"><strong><?php _e('End Date', 'cforms'); ?></strong></div>
					...
					<div class="optionsboxR">
                        ...
                        if( $dt=='x' && strlen($cformsSettings['form'.$no]['cforms'.$no.'_enddate'])>1 ):
							...
							if( $dt>0 ):
	                                echo '<label for="cforms_startdate">'.__('The form will be available for another ', 'cforms').sec2hms($dt).'.</label></div>';
	                            else:
	                                echo '<label for="cforms_startdate">'.__('The form is not available anymore.', 'cforms').'</label></div>';
							endif;
						endif;
                        ?>
					</div>
				...
				<?php if( $cformsSettings['form'.$no]['cforms'.$no.'_maxentries'] <> '' || $cformsSettings['form'.$no]['cforms'.$no.'_startdate'] <> '' || $cformsSettings['form'.$no]['cforms'.$no.'_enddate'] <> '' ) : ?>
					<div class="optionsbox">
						...
						<div class="optionsboxR">
							<div id="r3" class="rbox"><textarea rows="80px" cols="200px" name="cforms_limittxt" id="cforms_limittxt" ><?php echo stripslashes(htmlspecialchars($cformsSettings['form'.$no]['cforms'.$no.'_limittxt'])); ?></textarea><div id="rh3"></div></div>
						</div>
					</div>
				...
				<div class="optionsbox">
					...
					<div class="optionsboxR">
						<input type="radio" id="cforms_redirect"  name="cforms_redirect" value="0" <?php if($cformsSettings['form'.$no]['cforms'.$no.'_redirect']==0) echo "checked=\"checked\""; ?>/><label for="cforms_redirect"><?php _e('Disabled (default setting)', 'cforms'); ?></label><br />
						<input type="radio" id="cforms_redirect2" name="cforms_redirect" value="2" <?php if($cformsSettings['form'.$no]['cforms'.$no.'_redirect']==2) echo "checked=\"checked\""; ?>/><label for="cforms_redirect2"><?php _e('Hide form after successful submission', 'cforms'); ?></label><br />
						<input type="radio" id="cforms_redirect1" name="cforms_redirect" value="1" <?php if($cformsSettings['form'.$no]['cforms'.$no.'_redirect']==1) echo "checked=\"checked\""; ?>/><label for="cforms_redirect1"><?php _e('Enable alternative success page (redirect)', 'cforms'); ?></label><br />
				...
				<div class="optionsbox">
					<div class="optionsboxL"><label for="cforms_dontclear"><strong><?php _e('Don\'t reset fields', 'cforms'); ?></strong></label></div>
					<div class="optionsboxR">
						<input type="checkbox" id="cforms_dontclear" name="cforms_dontclear" <?php if($cformsSettings['form'.$no]['cforms'.$no.'_dontclear']) echo "checked=\"checked\""; ?>/><label for="cforms_dontclear"><?php _e('Re-display user input after form submission.', 'cforms'); ?><br/><?php _e('(currently not available for Ajax submissions)', 'cforms');?></label>
					</div>
				</div>
				...
				<div class="optionsbox">
					...
					<div class="optionsboxR">
						<input type="checkbox" id="cforms_action" name="cforms_action" <?php if($cformsSettings['form'.$no]['cforms'.$no.'_action']) echo "checked=\"checked\""; ?>/><label for="cforms_action"><?php _e('Enable alternative form action!', 'cforms'); ?></label><br />
				...
				<p class="infotxt ex" id="it2"><?php _e('If you enable an alternative <strong>form action</strong> you <u>will loose any cforms application logic</u> (spam security, field validation, DB tracking etc.) in non-ajax mode! This setting is really only for developers that require additional capabilities around forwarding of form data and will turn cforms into a front-end only, a "form builder" so to speak.', 'cforms') ?></p>

				<div class="optionsboxSEP"></div>

				<div class="optionsbox">
					<div class="optionsboxL"><label for="cforms_rss"><strong><?php _e('Enable RSS', 'cforms'); ?></strong></label></div>
					<div class="optionsboxR"><input type="checkbox" id="cforms_rss" name="cforms_rss" <?php if( $cformsSettings['form'.$no]['cforms'.$no.'_rss'] ) echo "checked=\"checked\""; ?>/> <?php _e('Enables an RSS feed to track new submissions.', 'cforms'); ?></div>
				</div>

				<?php if( current_user_can('track_cforms') && $cformsSettings['form'.$no]['cforms'.$no.'_rss'] ) : ?>
				<div class="optionsbox">
					<div class="optionsboxL"><label for="cforms_rsscount"><strong><?php _e('RSS entries', 'cforms'); ?></strong></label></div>
					<div class="optionsboxR"><input name="cforms_rsscount" id="cforms_rsscount" value="<?php echo $cformsSettings['form'.$no]['cforms'.$no.'_rss_count'];  ?>" /></div>
				</div>
				<div class="optionsbox">
					<div class="optionsboxL"><label for="cforms_rsskey"><strong><?php _e('RSS Feed Security Key', 'cforms'); ?></strong></label></div>
					<div class="optionsboxR">
		...
		<fieldset class="cformsoptions" id="emailoptions">
			...
			<div id="o3">
				...
				<div class="optionsbox" style="margin-top:30px;">
					<div class="optionsboxL"><label for="cforms_header"><?php _e('<strong>Admin TEXT message</strong><br />(Header)', 'cforms') ?></label></div>
					<div class="optionsboxRxl">
						<div id="r4" class="rbox"><textarea rows="80px" cols="200px" name="cforms_header" id="cforms_header" ><?php echo stripslashes(htmlspecialchars($cformsSettings['form'.$no]['cforms'.$no.'_header'])); ?></textarea><div id="rh4"></div></div>
				...
				<div class="optionsbox">
					<div class="optionsboxL"><?php _e('(Footer)','cforms')?></div>
					<div class="optionsboxR"><input type="checkbox" id="cforms_formdata_txt" name="cforms_formdata_txt" <?php if(substr($cformsSettings['form'.$no]['cforms'.$no.'_formdata'],0,1)=='1') echo "checked=\"checked\""; ?>/><label for="cforms_formdata_txt"><?php _e('<strong>Include</strong>  user input at the bottom of the admin email', 'cforms') ?></label></div>
				</div>
				<div class="optionsbox">
					<div class="optionsboxL">&nbsp;</div>
					<div class="optionsboxR"><input type="text" name="cforms_space" id="cforms_space" value="<?php echo stripslashes(htmlspecialchars($cformsSettings['form'.$no]['cforms'.$no.'_space'])); ?>" /><label for="cforms_space"> &nbsp; <?php _e('(# characters) : spacing between labels &amp; data, for plain txt version only', 'cforms') ?></label></div>
				</div>
				...
				<div class="optionsbox" style="margin-top:20px;">
					...
					<div class="optionsboxR"><input type="checkbox" id="cforms_admin_html" name="cforms_admin_html" <?php if($o=substr($cformsSettings['form'.$no]['cforms'.$no.'_formdata'],2,1)=='1') echo "checked=\"checked\""; ?>/></div>
				</div>

				<div class="optionsbox <?php if( !$o=='1' ) echo "hidden"; ?>">
					<div class="optionsboxL"><label for="cforms_header_html"><?php _e('<strong>Admin HTML message</strong><br />(Header)', 'cforms') ?></label></div>
					<div class="optionsboxRxl">
						<div id="r5" class="rbox"><textarea rows="80px" cols="200px" name="cforms_header_html" id="cforms_header_html" ><?php echo stripslashes(htmlspecialchars($cformsSettings['form'.$no]['cforms'.$no.'_header_html'])); ?></textarea><div id="rh5"></div></div>
						<span><?php echo sprintf(__('<a href="%s" %s>Variables</a> allowed.', 'cforms'),'?page='. $plugindir.'/cforms-help.php#variables','onclick="setshow(23)"'); ?></span>
					</div>
				</div>
				<div class="optionsbox <?php if( !$o=='1' ) echo "hidden"; ?>">
					<div class="optionsboxL"><?php _e('(Footer)','cforms')?></div>
					<div class="optionsboxR">
						<input type="checkbox" id="cforms_formdata_html" name="cforms_formdata_html" <?php if(substr($cformsSettings['form'.$no]['cforms'.$no.'_formdata'],1,1)=='1') echo "checked=\"checked\""; ?>/><label for="cforms_formdata_html"><?php _e('<strong>Include</strong> user input at the bottom of the admin email', 'cforms') ?></label>
						<br style="clear:both" /><br />
						<p><a class="infobutton" href="#" name="it3"><?php _e('\'Don\'t like the default (blue) form data block in your admin email?  &raquo;', 'cforms'); ?></a></p>
					</div>
					<p class="infotxt ex" id="it3"><strong><u><?php _e('Note:','cforms')?></u></strong> <?php _e('To avoid sending ALL of the submitted user data (especially for very long forms) to the form admin simply <strong>uncheck</strong> "<em>Include user input ...</em>" and instead specify the fields you\'d like to receive via the use of <strong>custom variables</strong>.', 'cforms'); ?></p>
				</div>
		...
		<fieldset class="cformsoptions <?php if( !$ccboxused ) echo "hidden"; ?>" id="cc">
			...
			<div id="o4">
				...
				<div class="optionsbox">
					...
					<div class="optionsboxR"><input type="text" name="cforms_ccsubject" id="cforms_ccsubject" value="<?php $t=explode('$#$',$cformsSettings['form'.$no]['cforms'.$no.'_csubject']); echo stripslashes(htmlspecialchars($t[1])); ?>" /> <?php echo sprintf(__('<a href="%s" %s>Variables</a> allowed.', 'cforms'),'?page='. $plugindir.'/cforms-help.php#variables','onclick="setshow(23)"'); ?></div>
				</div>
		...
		<fieldset class="cformsoptions" id="autoconf">
			...
			<div id="o5">
				...
				<div class="optionsbox">
					<div class="optionsboxL"></div>
					<div class="optionsboxR">
						<input type="checkbox" id="cforms_confirm" name="cforms_confirm" <?php if($o=$cformsSettings['form'.$no]['cforms'.$no.'_confirm']=="1") echo "checked=\"checked\""; ?>/><label for="cforms_confirm"><strong><?php _e('Activate auto confirmation', 'cforms') ?></strong></label>
				...
				<div class="optionsbox <?php if( !$o=="1" ) echo "hidden"; ?>">
					...
					<div class="optionsboxR"><input type="text" name="cforms_csubject" id="cforms_csubject" value="<?php $t=explode('$#$',$cformsSettings['form'.$no]['cforms'.$no.'_csubject']); echo stripslashes(htmlspecialchars($t[0])); ?>" /> <?php echo sprintf(__('<a href="%s" %s>Variables</a> allowed.', 'cforms'),'?page='. $plugindir.'/cforms-help.php#variables','onclick="setshow(23)"'); ?></div>
				</div>
				<div class="optionsbox <?php if( !$o=="1" ) echo "hidden"; ?>" style="margin-top:10px;">
					<div class="optionsboxL"><label for="cforms_cmsg"><strong><?php _e('TEXT message', 'cforms') ?></strong></label></div>
					<div class="optionsboxRxl">
						<div id="r6" class="rbox"><textarea rows="80px" cols="200px" name="cforms_cmsg" id="cforms_cmsg" ><?php echo stripslashes(htmlspecialchars($cformsSettings['form'.$no]['cforms'.$no.'_cmsg'])); ?></textarea><div id="rh6"></div></div>
						<span><?php echo sprintf(__('<a href="%s" %s>Variables</a> allowed.', 'cforms'),'?page='. $plugindir.'/cforms-help.php#variables','onclick="setshow(23)"'); ?></span>
 					</div>
				</div>
				<div class="optionsbox <?php if( !$o=="1" ) echo "hidden"; ?>" style="margin-top:15px;">
					...
					<div class="optionsboxR"><input type="checkbox" id="cforms_user_html" name="cforms_user_html" <?php if($o2=substr($cformsSettings['form'.$no]['cforms'.$no.'_formdata'],3,1)=='1') echo "checked=\"checked\""; ?>/></div>
				</div>
				<div class="optionsbox <?php if( !$o=="1" || !$o2=="1") echo "hidden"; ?>">
					<div class="optionsboxL"><label for="cforms_cmsg_html"><strong><?php _e('HTML message', 'cforms') ?></strong></label></div>
					<div class="optionsboxRxl">
						<div id="r7" class="rbox"><textarea rows="80px" cols="200px" name="cforms_cmsg_html" id="cforms_cmsg_html" ><?php echo stripslashes(htmlspecialchars($cformsSettings['form'.$no]['cforms'.$no.'_cmsg_html'])); ?></textarea><div id="rh7"></div></div>
						<span><?php echo sprintf(__('<a href="%s" %s>Variables</a> allowed.', 'cforms'),'?page='. $plugindir.'/cforms-help.php#variables','onclick="setshow(23)"'); ?></span>
					</div>
				</div>
		...
		<fieldset class="cformsoptions" id="tellafriend">
			...
			<div id="o6">
				...
				<div class="optionsbox">
					<div class="optionsboxL"></div>
					<div class="optionsboxR"><input type="checkbox" id="cforms_tellafriend" name="cforms_tellafriend" <?php if( substr($cformsSettings['form'.$no]['cforms'.$no.'_tellafriend'],0,1)=='1' ) echo "checked=\"checked\""; ?>/><label for="cforms_tellafriend"><strong><?php _e('Enable Tell-A-Friend', 'cforms') ?></strong></label></div>
				</div>

				<?php if( substr($cformsSettings['form'.$no]['cforms'.$no.'_tellafriend'],0,1)=='1' ) : ?>
				<div class="optionsbox">
					<div class="optionsboxL"></div>
					<div class="optionsboxR"><input type="checkbox" id="cforms_tafdefault" name="cforms_tafdefault" <?php if( substr($cformsSettings['form'.$no]['cforms'.$no.'_tellafriend'],1,1)=='1' ) echo "checked=\"checked\""; ?>/><label for="cforms_tafdefault"><strong><?php _e('T-A-F enable <strong>new posts/pages</strong> by default', 'cforms') ?></strong></label></div>
				</div>

				<div class="optionsbox" style="padding-top:25px;">
					<div class="optionsboxL"><label for="migrate"><?php _e('<strong>Batch T-A-F dis-/enable</strong><br />all your previous posts.', 'cforms') ?></label></div>
					<div class="optionsboxR">
					...
					<p class="infotxt ex" id="it9"><?php echo __('You will find a <strong>cforms Tell-A-Friend</strong> checkbox on your <strong>admin/edit page</strong> (typically under "Post/Author")! <br /><u>Check it</u> if you want to have the form to appear on the given post or page.', 'cforms');?></p>
				</div>
		...
		<fieldset class="cformsoptions" id="commentrep">
			...
			<div id="o7">
				...
				<div class="optionsbox">
					<div class="optionsboxL"><label for="cforms_commentrep"><strong><?php _e('WP comment form', 'cforms') ?></strong></label></div>
					<div class="optionsboxR"><input type="checkbox" id="cforms_commentrep" name="cforms_commentrep" <?php if( substr($cformsSettings['form'.$no]['cforms'.$no.'_tellafriend'],0,1)=='2' ) echo "checked=\"checked\""; ?>/><?php _e('Enable this form to optionally (user determined) act as a WP comment form', 'cforms') ?></div>
				</div>
				<div class="optionsbox">
					<div class="optionsboxL"><label for="cforms_commentXnote"><strong><?php _e('Comment notification', 'cforms') ?></strong></label></div>
					<div class="optionsboxR"><input type="checkbox" id="cforms_commentXnote" name="cforms_commentXnote" <?php if( substr($cformsSettings['form'.$no]['cforms'.$no.'_tellafriend'],1,1)=='1' ) echo "checked=\"checked\""; ?>/><?php _e('Extra comment notification (check your autoresponder setting, too!)', 'cforms') ?>
		...
		<fieldset class="cformsoptions" id="readnotify">
			...
			<div id="o8">
				...
				<div class="optionsbox">
					...
					<div class="optionsboxR"><input type="text" id="cforms_tracking" name="cforms_tracking" value="<?php echo stripslashes(htmlspecialchars($cformsSettings['form'.$no]['cforms'.$no.'_tracking'])); ?>"/></div>
				</div>
