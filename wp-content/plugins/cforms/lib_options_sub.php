<?php
	...
	for($i = 1; $i <= $field_count; $i++) {
	  ...
	  if ($_REQUEST['field_' . $i . '_name']<>''){    // Newly "AddField" does not exist yet!
				...
				if ($allgood)
						$cformsSettings['form'.$no]['cforms'.$no.'_count_field_' . $i] = $name . '$#$' . $type . '$#$' . $required. '$#$'. $emailcheck . '$#$'. $clear . '$#$'. $disabled . '$#$'. $readonly;
...
/*	if ( $_REQUEST['cforms_redirect']=='2' ){
		...
		$cformsSettings['form'.$no]['cforms'.$no.'_showpos'] = 'yn'.$tmp ;
	}
	else
*/
	$cformsSettings['form'.$no]['cforms'.$no.'_showpos'] =   ($_REQUEST['cforms_showposa']?'y':'n').($_REQUEST['cforms_showposb']?'y':'n').($_REQUEST['cforms_errorLI']?'y':'n').($_REQUEST['cforms_errorINS']?'y':'n').($_REQUEST['cforms_jump']?'y':'n') ;
	...
	$cformsSettings['form'.$no]['cforms'.$no.'_formdata'] =     ($_REQUEST['cforms_formdata_txt']?'1':'0').($_REQUEST['cforms_formdata_html']?'1':'0').($_REQUEST['cforms_admin_html']?'1':'0').($_REQUEST['cforms_user_html']?'1':'0') ;
	...
	$cformsSettings['form'.$no]['cforms'.$no.'_startdate'] = preg_replace("/\\\+/", "\\",$_REQUEST['cforms_startdate']).' '.preg_replace("/\\\+/", "\\",$_REQUEST['cforms_starttime']);
    $cformsSettings['form'.$no]['cforms'.$no.'_enddate'] =  preg_replace("/\\\+/", "\\",$_REQUEST['cforms_enddate']).' '.preg_replace("/\\\+/", "\\",$_REQUEST['cforms_endtime']);
